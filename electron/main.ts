...
ipcMain.handle('cloud:syncUsers', async () => {
    try {
        const setting = await prisma.setting.findUnique({ where: { key: 'CLOUD_API_URL' } });
        if (!setting || !setting.value) {
            return { success: false, error: 'Cloud API URL not configured in Settings.' };
        ...
        const users = await prisma.user.findMany();
        const result = await cloudSync.syncUsers(users);
        if (!result?.success) {
            return { success: false, error: result?.error || 'User sync failed' };
        }

        return { success: true, count: users.length };
    } catch (error: any) {
        console.error('User-only sync failed:', error);
        return { success: false, error: error?.message || 'User-only sync failed' };
    }
});

ipcMain.handle('cloud:setDashboardUser', async (_event, payload: { username: string; password: string; name?: string; role?: string }) => {
    try {
        const setting = await prisma.setting.findUnique({ where: { key: 'CLOUD_API_URL' } });
        if (!setting || !setting.value) {
            return { success: false, error: 'Cloud API URL not configured in Settings.' };
        ...
        cloudSync.setApiUrl(setting.value);
        const result = await cloudSync.setDashboardUser(payload);
        if (!result?.success) {
            return { success: false, error: result?.error || 'Failed to set dashboard user' };
        }

        return { success: true };
    } catch (error: any) {
        console.error('Set dashboard user failed:', error);
        return { success: false, error: error?.message || 'Set dashboard user failed' };
    }
});
